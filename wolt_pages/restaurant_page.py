...
class RestaurantPage(BasePage):
    ...
    def verify_restaurant_name(self, restaurant_name):
        """Verify the restaurant name on the card is same as in the card title in all restaurants page after entering,
        for validation."""
    ...
    def add_food_items_on_specified_sections(self, start=2, section_end=3):
        """
        ...
        self.logger.info(f"Adding food items from sections {start} to {start + section_end - 1}.")

        for section_number in range(start, start + section_end):
            # Replace `number` in the template to locate the section
